...
role_change_dates = [config.start_day + datetime.timedelta(days=day_offset) for day_offset in role_change_date_offsets]
for i, user_index in enumerate(role_change_candidates):
    ...
    daily_time_sheet[
        (daily_time_sheet['UserID'] == user_id) &
        (daily_time_sheet['Day'] > role_change_dates[i])
        ] = config.roles.remove(user_role)[randint(len(config.roles) - 1)]

'''
distribute role shifts according to daily employee's roles.
'''
for day in daily_time_sheet['Day'].unique():
    all_shifts_of_the_day_index = daily_time_sheet[
        (daily_time_sheet['Day'] == day) &
        (daily_time_sheet['Rol'].isin(['Support Shift', 'Regular Shift']))
        ].index
    daily_time_sheet.loc[all_shifts_of_the_day_index, 'Shift'] = \
...
'''
days_of_shift_change = np.random.randint(0, config.duration_days, size=randint(3, config.duration_days))
...
for i, day_offset in enumerate(days_of_shift_change):
    shift_change_idxs = daily_time_sheet[
        (daily_time_sheet['Day'] == config.start_day + datetime.timedelta(days=day_offset)) &
        (daily_time_sheet['Rol'].isin(['Support Shift', 'Regular Shift']))
        ].index
    ...
    daily_time_sheet.iloc[shift_change_idxs]['Shift'] = \
        (daily_time_sheet.iloc[shift_change_idxs]['Shift'] + t_number_of_shifts_to_change % 3)
